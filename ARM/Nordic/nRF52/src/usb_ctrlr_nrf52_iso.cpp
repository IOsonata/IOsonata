...
Kept in its own archive member so applications without UsbIsoIntrf do not
pull the ISO scheduler, SOF processing or deferred completion path.
----------------------------------------------------------------------------*/
#include <stddef.h>
#include <stdint.h>
...
static inline __attribute__((always_inline))
void nRFIsoRegisteredEvent(uint8_t EpAddr, UsbCtrlrEvtType_t Event,
...
static void nRFUsbdProcessIsoComplete(uint32_t Evt, void *pContext)
{
	...
	nRFIsoRegisteredEvent(epAddr, USB_CTRLR_EVT_XFER_CMPL,
		...
		if ((flags & USBD_FLAG_ISO_OUT_BUSY) == 0U || waiting)
		{
			if (size != 0U)
			{
				...
				if (!waiting)
				{
					...
					if (pReg->bBlocking)
					{
						nRFIsoRegisteredEvent(NRFX_USBD_ISO_EP_NO,
...
bool nRFUsbdIsoEpOpen(const UsbEndPointDesc_t *pDesc)
{
	...
	const uint8_t endBit = in ?
		USBD_INTEN_ENDISOIN_Pos :
		(USBD_INTEN_ENDEPOUT0_Pos + NRFX_USBD_ISO_EP_NO);
	*pEnd = 0U;
	NRF_USBD->INTENSET = 1UL << endBit;
	*pEnable |= 1UL << NRFX_USBD_ISO_EP_NO;
