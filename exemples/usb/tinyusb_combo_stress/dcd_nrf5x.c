...
bool dcd_edpt_xfer(uint8_t rhport, uint8_t ep_addr, uint8_t* buffer, uint16_t total_bytes, bool is_isr) {
  ...
  if (control_status) {
    ...
    // ISO IN is staged now and admitted at SOF. Completing ENDISOIN frees
    // the TinyUSB endpoint immediately so the class can stage the next frame
    // before the following SOF without overwriting the current endpoint data.
    xfer->started = true;
