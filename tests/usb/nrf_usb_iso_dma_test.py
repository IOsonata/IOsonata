...
struct {
 ...
 uint16_t EpOutPacketSize[9]={};uint16_t EpOutBlocking=0;uint16_t IsoInPacketSize=0;
...
void init(){
 ...
 memset(s_Usbd.EpOutPacketSize,0,sizeof(s_Usbd.EpOutPacketSize));s_Usbd.EpOutBlocking=0;s_Usbd.IsoInPacketSize=0;
 ...
 s_Usbd.EpOutPacketSize[8]=512;s_Usbd.IsoInPacketSize=512;
...
int main(){
 ...
 s_Usbd.Flags|=USBD_FLAG_ISO_OUT_OPEN;s_Usbd.EpOutPacketSize[8]=9;frame(9);finish(false);
 ...
 for(unsigned ep=1;ep<8;++ep)for(unsigned dir=0;dir<2;++dir)for(unsigned masked=0;masked<2;++masked){
  ...
  for(unsigned n=0;n<8;++n){
   ...
   regs.SIZE.EPOUT[n]=64;s_Usbd.EpOutPacketSize[n]=64;
  }
  ...
  for(unsigned n=0;n<8;++n){
   ...
   assert(s_Usbd.EpOutPacketSize[n]==((n==ep&&!dir)?0U:64U));
  }
 }
