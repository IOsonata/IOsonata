...
if [[ "$MODE" != "force" && -d "$ECLIPSE_APP" ]]; then
  echo "✅ Eclipse already installed at $ECLIPSE_APP (skipping; use --force-update to reinstall)."
else
...
seed_eclipse_prefs() {
  ...
  local JRE
  JRE=$(/usr/libexec/java_home 2>/dev/null || true)
  if [[ -z "$JRE" ]]; then
...
# =========================================================
BUILD_SCRIPT="$ROOT/IOsonata/tools/build_iosonata_lib_macos.sh"
