...
@brief	CryptoDev_t hardware engine on the Arm CC3xx low level driver
		...
		with no vendor blob, no PSA core, no nRF5 SDK. A library links exactly
		one file defining CryptoHwInit: this one, crypto_psa_bm.cpp, or
		crypto_hw_none.cpp.
		...
		The local cc3xx_rng.c routes the CC3xx random API to IOsonata RngGet.
