...
@brief	IOsonata identity manager for the sdk-nrf-bm SoftDevice path.
		...
		Replaces the sdk-nrf-bm peer_manager module id_manager.c while keeping
		the same im_* API. Peer Manager, the GATT cache manager and the security
		manager continue to call the existing interface unchanged.

		The module owns connection-to-peer mapping, peer address resolution,
		duplicate bond detection, master-ID lookup, allow-list state, device
		identities and privacy passthroughs. Bond records remain in the existing
		peer_database / peer_data_storage layer during this migration stage.

		S145 connection handles are mapped through nrf_sdh_ble_idx_get; they are
		not assumed to be dense. Resolvable private addresses are never tested
		against an all-zero IRK, and the cached allow list is updated only after
		the SoftDevice accepts the new list.
...
static bool BondIterPrepare(struct pm_peer_data_const *pPeerData,
							uint8_t *pBuffer, uint16_t *pIter)
{
	if (pPeerData == nullptr || pBuffer == nullptr || pIter == nullptr)
	{
		return false;
	}
							   ...
							   ble_gap_irk_t *pIrks, uint32_t *pIrkCnt)
{
	...
	for (uint32_t i = 0; i < PeerCnt; i++)
	{
		...
		{
			return (status == NRF_ERROR_INVALID_PARAM) ?
				   NRF_ERROR_NOT_FOUND : status;
		}
