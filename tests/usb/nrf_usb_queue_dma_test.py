...
void init(){
 ...
 s_Usbd.SofEnabled=false;s_Usbd.IsoBufState=0;
...
int main(int argc,char **argv){
 ...
 for(int length:{1,64,65,256}){
  ...
  while(offset<length){
   auto *packet=(nRFEPPkt_t*)CFifoPeek(s_Usbd.hEp0Que);
   const int bytes=min(length-offset,64);
   assert(packet && packet->Len==bytes && !ep0Completions);
   assert(regs.EPIN[0].PTR==uint32_t(uintptr_t(packet->Payload)));
   for(int i=0;i<bytes;++i)assert(packet->Payload[i]==uint8_t(offset+i));
   regs.EPSTATUS.bits=1;regs.EVENTS_ENDEPIN[0]=regs.EVENTS_EP0DATADONE=1;
   interrupt();offset+=bytes;
  }
  assert(ep0Completions==1 && ep0Length==0);
 ...
 for(unsigned gate:{0U,unsigned(USBD_FLAG_SUSPENDED),unsigned(USBD_FLAG_HOST_RESUME),
  ...
  if(status==2U)assert(UsbCtrlrEpSend(0,1,data,9));
   ...
   dmaBusy=0x82;
   s_Usbd.EpReg[7][0].IsoOpen=true;s_Usbd.EpReg[7][0].MaxPacketSize=s_Usbd.EpReg[7][1].MaxPacketSize=64;
  }
 ...
 for(unsigned gate:{0U,unsigned(USBD_FLAG_SUSPENDED),unsigned(USBD_FLAG_HOST_RESUME),
  ...
  init();dmaBusy=0x82;
  if(status==2U)assert(UsbCtrlrEpSend(0,1,data,9));
  else { s_Usbd.EpReg[7][0].IsoOpen=true; s_Usbd.EpReg[7][0].MaxPacketSize=s_Usbd.EpReg[7][1].MaxPacketSize=64; }
  assert(UsbCtrlrEpSend(0,2,data+64,9));
  s_Usbd.Flags|=gate;
  regs.EPSTATUS.bits=status;regs.EVENTS_ENDEPIN[1]=1;isoEnd=1;
  interrupt();
 ...
 for(unsigned mask:{0U,1U}){
  ...
  assert(s_Usbd.EpReg[7][0].IsoOpen);
  nRFUsbdHostResumeDetected();
