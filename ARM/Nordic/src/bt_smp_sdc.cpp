...
Targeted provider (bt_smp_<plat>.cpp) for the SoftDevice Controller. Exports
g_BtSmpCryptoSdc; the application installs it via BtAppCfg_t.pSmpCrypto ->
BtSmpInit. The generic bt_smp.cpp dispatches all crypto through the installed
provider, so this file is compiled into the SDC lib alongside any other
provider without symbol collision (the implementations are file-static; only
the const vtable instance is exported).
...
// Platform availability guard (NOT feature selection): on a SoftDevice target
// sdc_hci_cmd_le.h does not exist, so this provider compiles to nothing there.
// The SDC lib always has the header; this is just so the shared source tree
// stays buildable across Nordic backends.
...
//-----------------------------------------------------------------------------
// Provider implementation (file-static; exported only via g_BtSmpCryptoSdc)
//-----------------------------------------------------------------------------

static void SdcRand(uint8_t *pBuf, size_t Len)
{
...
// micro-ecc RNG adapter. uECC wants int(dest,size)->1 on success; back it with
// the controller LE Rand.
...
static void SdcAes128(BtHciDevice_t * const pDev,
...
static int SdcP256KeyGen(BtHciDevice_t * const pDev, uint8_t pPubKey[64])
{
	(void)pDev;

	// micro-ecc needs an RNG registered before make_key. Back it with the
	// controller LE Rand. Register once.
...
static int SdcEcdh(BtHciDevice_t * const pDev,
...
extern "C" const BtSmpCrypto_t g_BtSmpCryptoSdc = {
	"sdc",
	SdcAes128,
	SdcP256KeyGen,
	SdcEcdh,
